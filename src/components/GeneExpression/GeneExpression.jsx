...
const GeneExpression = ({ formattedData }) => {
  ...
  const [cellType, setCellType] = useState("B Naive");
  ...
  useEffect(() => {
    ...
    const geneValue = typeof selectedGene === "string" ? selectedGene : selectedGene.value;
      ...
      return Object.entries(study.celltypes)
        .map(([celltype, data]) => {
          ...
          return {
            ...
            celltype: celltype,
            geneName: geneValue,
            pvalue: geneIndex !== -1 ? data.pvalue[geneIndex] : null,
            log2FoldChange: geneIndex !== -1 ? data.log2FoldChange[geneIndex] : null,
          };
        })
        .filter((d) => d.pvalue !== null);
    });

    // Calculate dynamic dimensions based on data
    const margin = { top: 50, right: 150, bottom: 80, left: 140 };
    const minWidth = 600;
    const minHeight = 400;
    
    // Calculate space needed per item
    const uniqueCellTypes = Array.from(new Set(plotData.map((d) => d.celltype)));
    const uniqueStudyNames = Array.from(new Set(plotData.map((d) => d.studyName)));
    ...
    const sizeScale = d3
      ...
      .range([3, 12]); // Slightly larger circles for better visibility
    ...
    // Improved axis labels with consistent styling
    svg
      .append("text")
      .attr("transform", `rotate(-90)`)
      .attr("x", -(margin.top + height/4))
      .attr("y", margin.left/4 + 50)
  ...
  return (
    <Flex direction="column" align="center" w="100%" h="100%" maxW="1200px" p={4}>
      {isLoading ? (
          ...
          >
            ...
            >
              <FormControl w="100%" ref={searchRef}>
                ...
                {showDropdown &&
                  (filteredGenes.length > 0 || isInputFocused) && (
                    <List
                      position="absolute"
                      top="100%"
                      left={0}
            ...
            <Flex
              w="30%" // Adjust width to take up 30% of the row
              minW="250px"
              direction="column"
            >
              <FormLabel fontWeight="bold" color="gray.700">
